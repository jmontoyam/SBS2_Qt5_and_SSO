...
void loadData(QString pathFile, DTU::DtuArray2D<double> *matrix)
{
    ...
    /*************************/
    QString outFile(Sbs2Common::getRootAppPath()+"outfile.txt");
    QFile fh_out(outFile);
    fh_out.open(QIODevice::WriteOnly|QIODevice::Text);

    QTextStream out(&fh_out);
    ...
    if(!fh.open(QIODevice::ReadOnly|QIODevice::Text))
    {
        ...
        out<<"Error while reading the file: "<<pathFile;
        out<<"\n";
        return;
    }

    out<<"The file has been loaded: "<<pathFile;
    out<<"\n";
    out.flush();
    fh_out.close();
